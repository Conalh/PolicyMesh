...
import type { Finding, MatrixRow, McpServer, RepoPolicies, Severity, SurfaceId } from '../types.js';
...
function detectRiskyInstructions(policies: RepoPolicies): Finding[] {
...
function severityFor(category: string): Severity {
  switch (category) {
...
function detectAiderDangerousAllowNonGit(policies: RepoPolicies): Finding[] {
 ...
 */
function mcpServerCellLabel(server: McpServer): string {
...
function extractPinnedVersion(command: string): string | undefined {
...
export function selectConflictRows(matrix: MatrixRow[]): MatrixRow[] {
  return matrix.filter((row) => {
    const values = Object.values(row.values).filter((value): value is string => Boolean(value));
    if (values.length < 2) {
      return false;
    }
 ...
 */
export function describePosture(policies: RepoPolicies): string | undefined {
...
function strictestSignal(policies: RepoPolicies): string | undefined {
...
function loosestSignal(policies: RepoPolicies): string | undefined {

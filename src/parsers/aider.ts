...
import type { AiderPolicy, Finding } from '../types.js';

const AIDER_CONFIG_FILE = '.aider.conf.yml';

interface AiderParseResult {
  policy?: AiderPolicy;
  findings: Finding[];
}
...
export async function parseAiderPolicy(root: string): Promise<AiderParseResult> {
  let text = '';
  try {
 ...
 * YAML dependency.
 */
function parseTopLevelYaml(text: string): Map<string, ParsedEntry> {
